...
`config.json` 的 `defaultLargeScope` 決定預設用哪一個（`all` / `front`），**目前是 `front`**。
...
`.github/workflows/daily.yml` **排程已啟用**：週一到週五，台北時間 10:07 起大約每小時一次，
到 22:41 為止，共 10 個 cron。排程帶 `--days 7 --max-stale-days 5`
...
GitHub 的排程是 **best-effort**：所有人的排程共用同一批 runner，負載高的時候會延遲，
甚至整個丟掉而且不留任何紀錄。本 repo 實測（2026/09/15 ~ 09/18，換算台北時間）：

| 設定 | 9/15 | 9/16 | 9/17 | 9/18 | 實際延遲 |
|---|---|---|---|---|---|
| 16:17 | 21:32 | 21:29 | 21:26 | 20:57 | **+5 小時** |
| 18:23 | 23:05 | 23:02 | 23:05 | 22:33 | **+4.7 小時** |
| 22:41 | 隔日 02:28 | 02:25 | 02:32 | 01:52 | **+3.8 小時** |

**把分鐘數從整點移開並沒有用**——`0 8` 改成 `17 8` 之後延遲幅度完全一樣。
延遲以小時計的時候，錯開一分鐘不會有任何幫助。

既然延遲量控制不了，就改成**撒一排**：從台北 10:07 到 18:23 每小時左右一個，
不管當天的延遲是 0 還是 8 小時，總有一個會落在想要的 16:00~18:00。

**前提是「跑了不一定要做事」，而這點成立：**

* 資料還沒公布 → 期交所回 0 筆 → `parse_fut` 立刻丟 `NoDataForDate`，
  **只花 1 次對外請求**就結束（後面兩個端點根本不會被呼叫）。
* 當天已經抓完 → `--days 7` 的回看範圍全部完整 → **一次對外請求都不發**
  （`tests/test_update_daily.py` 第 9 條釘住這件事）。

所以多跑幾次幾乎不花成本，而且腳本本來就是冪等的。
分鐘數全部避開整點（第 11 條測試釘住這件事——整點是最壅塞的一分鐘）。

**這只是提高命中率，不是保證。** 真的需要準時，就不能靠 GitHub 的 cron，
得改用外部排程器透過 API 觸發 `workflow_dispatch`，或在一台固定開著的機器上跑。

另外兩件要知道的事：

* 就算某天整個被丟掉，資料也不會掉——`--days 7` 會在下一次成功的排程把它補回來。
  最壞情況是網站晚一天，不會出現永久的洞。
...
三種計分模式（逐日和另一份獨立實作比對，並確認暖身天數與曲線設定真的生效）、
釘住當日明細的開關行為，還有偏好記憶（真的 reload 一次，並驗存到無效值時會退回預設）。

## 五票規則

三種計分模式共用同一個尺度：**每欄 -1 ~ +1、總分 -5 ~ +5**。尺度一致是刻意的，
換模式時圖表的軸、背景色塊、統計卡都不必跟著改。用 `config.json` 的
`defaultScoreMode` 決定預設（`level` / `flow` / `blend`）。
...
期貨小計 = 票 1+2+3，選擇權小計 = 票 4+5，總分 = 五票相加。
控制列會一直顯示兩個預設門檻判讀不同的是哪幾天。

### 模式 2：今日操作（`flow`）

同樣五張票，但改看**今日操作**（今日口數 − 昨日口數）的正負。
歷史第一天沒有前一日，顯示 `—`（不計入統計）。

### 模式 3：加權（`blend`）

每欄拆成**水位分 0.5 ＋ 操作分 0.5**，分數連續：

```
u      = (今天的值 − 中性點) ÷ 窗內穩健標準差        ← 「是平常波動的幾倍」
水位分 = curve(u) × 0.5
操作分 = curve(今日操作 ÷ 操作的穩健標準差) × 0.5
每欄   = 水位分 + 操作分  ∈ [-1, +1]
```

三個設計決定，都是先看 2026/05~09 的實際分布才定的：

**中性點分欄設定。** `foreign_fut` 在 87 天裡**一天都沒有正過**（平均 -75,407，
範圍 -90,038 ~ -42,744），`foreign_opt` 也只有 5%。對這兩欄用 0 當中性點的話，
水位分會永遠卡在 -0.41 ~ -0.50、標準差只有 **0.026**——那是常數不是分數，那一票等於失效。
所以這兩欄改用**窗內中位數**當中性點；其餘三欄（實際上會繞著 0 擺動，>0 的天數
48%~63%）維持用 **0**。這也正是第 1 票當初要設可調門檻的原因：它的中性點本來就不是 0。

**尺度用 MAD×1.4826，不是標準差。** 籌碼資料厚尾，而且結算換月會跳空；
用標準差的話，一天的極端值會把之後整個視窗的分數都壓縮掉。

**曲線用 `tanh`，不封頂。** 20 日視窗的 MAD 只有 20 個樣本，會低估離散度，
所以 `clamp 2σ` 會有 23% 的天數頂到 ±0.5——2026/09/01（平常波動的 4.51 倍）和
08/26（3.29 倍）畫面上會變成同一個分數。`tanh(u/2.5)` 頂到 ±0.45 的只有 5%，
而且永遠保留排序（+0.474 vs +0.433）。`scoreCurve` 可改成 `clamp2` / `clamp3`。

視窗長度由 `scoreWindow` 決定（預設 20）。前 `scoreWindow + 1` 天暖身不足，
顯示 `—` 而不是 0——0 是「多空剛好抵銷」，和「算不出來」是兩回事。
視窗改 60 的話飽和率會再降（`clamp 2σ` 從 23% 降到 8%），但可計分的天數也會少 40 天。

**已知的取捨：水位和操作並不獨立**（水位就是操作的累加）。實測相關係數
`foreign_fut` +0.03、十大 +0.28/+0.26、選擇權 +0.44/+0.46——選擇權那兩票的
0.5/0.5 平分其實偏重了當日動能。目前先維持平分，之後要調再說。

**門檻只有模式 1 會用到。** 模式 2、3 會把門檻那一塊停用並說明原因，
免得以為調了有效果。
...
**點 K 線可以把當日明細釘住**，再點同一天、按面板右上角的 ×、或按 Esc 都會收起來；
點另一天就換成那天。釘住時會關掉 hover 的浮動提示，不然兩張一樣的卡片會疊在一起。

釘的是**日期**而不是索引——切區間之後同一個索引會指到別天，存索引的話面板會安靜地
換成另一天的資料，那種錯很難發現。釘住的那天不在目前視窗內時，面板收起來而不是顯示錯的。
面板內容直接重用 tooltip 那份 HTML，所以釘住的和 hover 看到的永遠一致。

**顯示區間與主要標的會記在 localStorage**（key `chips-kline:ui`），重新整理後還在。
讀回來的值一律要驗過才用——存過的股票代碼可能已經從 config 移除、區間選項也可能改過，
驗不過就安靜回到預設。門檻與十大口徑**刻意不記**：那兩個會改變判讀結果，
應該以 `config.json` 為準，不然哪天不小心點過一次，之後每次打開看到的都是自己忘了設過的東西。
