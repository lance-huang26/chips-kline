...
`.github/workflows/daily.yml` **排程已啟用**：週一到週五台北時間 16:17、18:23、22:41
（cron 是 UTC 的 `17 8`、`23 10`、`41 14`）。排程帶 `--days 7 --max-stale-days 5`
...
* **GitHub 的排程是 best-effort，可能延遲數小時，也可能整天不派送。**
  本 repo 實測 `0 8` / `0 10` 每天都被延後約四小時（台北 20:30 / 21:55 才跑），
  2026/09/14 更是一次都沒派送。整點是最壅塞的一分鐘，所以現在刻意挑奇數分鐘。
* **所以排程回看一週，而不是只抓當天。** 只抓當天的話，漏掉的那天會變成永久的洞，
  而且沒人會發現——隔天的排程照樣是綠的。回看一週幾乎不花成本：已經完整的日子
  一個請求都不發（`tests/test_update_daily.py` 第 9 條釘住這件事），
  漏掉的那天會在下一次排程自己補回來。真的連漏三次以上，就手動 Run workflow。
