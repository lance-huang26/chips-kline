...
fixture 是 2026/07/13、08/19、08/31 三天的真實回應。
08 月兩天的期望值是專案原本手工維護的 chips.csv；07/13 是後來補的，
因為它的期貨表格帶有最後那個「期貨小計」區塊——第一次在 GitHub Actions 上
跑回填就是死在那裡。期交所改版時，這裡會先紅，而不是等到資料已經寫髒了才發現。
...
# 「期貨小計」陷阱：那是所有期貨契約的加總，不能當成臺股期貨。
# 2026/09 第一次在 GitHub Actions 上跑就是死在這個區塊（前綴只有 1 格）。
d713 = "2026/07/13"
sub = taifex.parse_fut(fixtures[d713]["fut_rows"], d713)
check(sub == -81066, "%s foreign_fut 應該是臺股期貨的 -81,066，實得 %s" % (d713, sub))
check(sub != -561385,
      "抓到「期貨小計」的外資淨額了——那是所有期貨契約的加總，數字看起來像回事但整個判讀都會錯")

# 只剩小計區塊（沒有臺股期貨）→ 必須報錯，不能默默回小計的數字
expect_raises(taifex.TaifexError,
              lambda: taifex.parse_fut([r for r in fixtures[d713]["fut_rows"]
                                        if "臺股期貨" not in r], d713),
              "表格裡沒有臺股期貨時應該報錯，而不是退而求其次抓小計")
print("  期貨小計陷阱：%s 取到臺股期貨的 -81,066，沒有誤抓小計的 -561,385" % d713)
...
if fails:
    ...
    for f in fails:
        print("   -", f)
    sys.exit(1)
print("✅ 期交所解析器全部通過（%d 個日期 × 5 欄，含期貨小計陷阱、HTML 抽列與失敗路徑）"
      % len(fixtures))
